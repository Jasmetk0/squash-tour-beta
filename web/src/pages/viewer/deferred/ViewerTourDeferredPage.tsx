...
export function ViewerTourDeferredPage({
  ...
  const runQuery = useQuery({
    queryKey: ['viewer-tour-deferred-run', kind, activeRunId],
    queryFn: () => getRun(activeRunId ?? ''),
    enabled: Boolean(activeRunId),
    retry: false,
  })
  const statusQuery = useQuery({
    queryKey: ['viewer-tour-deferred-status', kind, activeRunId],
    queryFn: () => getRunStatusSummary(activeRunId ?? ''),
    enabled: Boolean(activeRunId),
    retry: false,
  })
  const eventsQuery = useQuery({
    queryKey: ['viewer-tour-deferred-events', kind, activeRunId],
    queryFn: () => listEvents(activeRunId ?? ''),
    enabled: Boolean(activeRunId),
    retry: false,
  })
  const rankingSnapshotsQuery = useQuery({
    queryKey: ['viewer-tour-deferred-ranking-snapshots', kind, activeRunId],
    queryFn: () => listRankingSnapshots(activeRunId ?? ''),
    enabled: Boolean(activeRunId),
    retry: false,
  })
  const raceSnapshotsQuery = useQuery({
    queryKey: ['viewer-tour-deferred-race-snapshots', kind, activeRunId],
    queryFn: () => listRaceSnapshots(activeRunId ?? ''),
    enabled: Boolean(activeRunId),
    retry: false,
  })
  const finalsQuery = useQuery({
    queryKey: ['viewer-tour-deferred-finals', kind, activeRunId],
