...
CareerStage = Literal[
    "junior", "developing", "breakthrough", "prime", "veteran", "late_career"
]
...
class InitialPoolPlayerUpdate(BaseModel):
    ...
    birth_year_week: int | None = Field(
        default=None, ge=1, le=DEFAULT_WEEKS_PER_CALENDAR_YEAR
    )
...
class InitialPoolMetadata(BaseModel):
    ...
    population_weighting_diagnostics: list[InitialPoolPopulationWeightingDiagnostic] = (
...
def initial_pool_age_weights() -> dict[int, float]:
    ...
    if total_weight <= 0:
        raise ValueError(
            "initial-pool age distribution must have positive total weight"
        )
    return {age: weight / total_weight for age, weight in sorted(age_weights.items())}


def initial_pool_effective_population_quantity(
    ...
    return initial_pool_effective_population_diagnostics(
        country, season_start_year
...
class InitialPlayerPoolGenerator:
    ...
    def generate(
        ...
        *,
        ...
        selected = self._filter_countries(
            countries, country_code=country_code, region=region
        )
        if not selected:
            raise ValueError(
                "initial pool generation requires at least one matching country"
            )
        locked = sorted(
            existing_locked_players or [], key=lambda player: player.player_id
        )
                ...
                if sequence not in locked_sequences[country.code]:
                    generated.append(
                        self._generate_player(
                            country=country, season=season, seed=seed, sequence=sequence
                        )
                    )
        ...
        players = sorted(
            [*locked, *generated],
            key=lambda player: (player.country_code, player.player_id),
        )
        ...
        return InitialPoolResult(
            ...
            metadata=InitialPoolMetadata(
                ...
                **self._population_weighting_metadata(
                    season, diagnostics=allocation_plan.diagnostics
                ),
            ),
        )
    ...
    def regenerate_unlocked(
        ...
        *,
        ...
        country_codes = {
            country.code
            for country in self._filter_countries(
                countries, country_code=country_code, region=region
            )
        }
        scoped_locked = [
            ...
            if player.country_code in country_codes and player.locked
        ]
        outside_scope = [
            player
            for player in current_players
            if player.country_code not in country_codes
        ]
        current_scope_count = sum(
            1 for player in current_players if player.country_code in country_codes
        )
        scope_target = (
            max(current_scope_count, len(scoped_locked))
            if target_pool_size is None
            else target_pool_size
        )
    ...
    def _locked_sequences_for_country(
        self, players: list[InitialPoolGeneratedPlayer], country_code: str
    ) -> set[int]:
    ...
    def _filter_countries(
        self, countries: list[Country], *, country_code: str | None, region: str | None
    ) -> list[Country]:
        items = countries
        if country_code:
    ...
    def _allocate_counts(
        self, countries: list[Country], target: int, *, seed: int, season: str
    ) -> dict[str, int]:
        return self._build_allocation_plan(
            countries, target, seed=seed, season=season
        ).counts
    ...
    def _build_allocation_plan(
        ...
        *,
        ...
        population_diagnostics = {
            country.code: initial_pool_effective_population_diagnostics(
                country, season_start_year
            )
            for country in countries
        }
        weights = {
            country.code: self._quantity_weight(
                country, season_start_year=season_start_year
            )
            for country in countries
        }
        ...
        if target > 0 and total > 0:
            counts = {
                country.code: int((weights[country.code] / total) * target)
                for country in countries
            }
            for country in countries:
            ...
            ranked = sorted(
                ...
                key=lambda c: (
                    -(
                        (weights[c.code] / total) * target
                        - int((weights[c.code] / total) * target)
                    ),
                    c.code,
                ),
            )
            ...
            diagnostics.append(
                base.model_copy(
                    update={
                        ...
                        "allocation_share": weights[country.code] / total
                        if total > 0
                        else 0.0,
                        "generated_allocation_count": counts[country.code],
                        "final_country_count": counts[country.code]
                        + locked_counts[country.code],
                    }
                )
            )
    ...
    def _generate_player(
        self, *, country: Country, season: str, seed: int, sequence: int
    ) -> InitialPoolGeneratedPlayer:
        rng = DeterministicRng(seed).branch(
            SeedScope.SEASON, "initial_pool", season, country.code, sequence
        )
        ...
        play_style = (
            self._identity().play_styles[
                self._identity().archetypes.index(archetype)
                % len(self._identity().play_styles)
            ]
            if archetype in self._identity().archetypes
            else rng.choice(self._identity().play_styles)
        )
    ...
    def _quantity_weight(self, country: Country, *, season_start_year: int) -> float:
        effective_quantity_population = self._effective_population_quantity(
            country, season_start_year
        )
        ...
        return max(
            0.000001,
            CountryTalentModel().effective_squash_pool_weight(
                country, effective_quantity_population
            ),
        )

    def _effective_population_quantity(
        self, country: Country, season_start_year: int
    ) -> float:
    ...
    def _current_ability(
        ...
        *,
        ...
        return max(
            1,
            min(99, potential + 4, int(round(potential * factor + rng.uniform(-4, 4)))),
        )

    def _attributes(
        self, rng: DeterministicRng, *, current: int, archetype: str
    ...
    def _summarize(
        ...
        return InitialPoolSummary(
            ...
            average_current_ability=round(
                sum(player.current_ability for player in players) / count, 2
            )
            if count
            else 0.0,
            average_potential_ability=round(
                sum(player.potential_ability for player in players) / count, 2
            )
            if count
            else 0.0,
            by_country=dict(
                sorted(Counter(player.country_code for player in players).items())
            ),
            by_career_stage=dict(
                sorted(Counter(player.career_stage for player in players).items())
            ),
            by_potential_tier=dict(
                sorted(Counter(player.potential_tier for player in players).items())
            ),
        )

    def _fingerprint(
        self, players: list[InitialPoolGeneratedPlayer], *, season: str, seed: int
    ) -> str:
        material = "|".join(
            [season, str(seed), *(player.model_dump_json() for player in players)]
        )
