...
| Season Closing Ranking | `season_closing_ranking.v1` calculates the archived Week-61 snapshot under the outgoing policy; seasons 0–48 persist Week-61 sources against next Season Week 1, while 2049/50 uses Closing-only owned source v6 with a post-final boundary ordinal and no fictional Week 1. Final 2049/50 now has an atomic command that stages the Closing Ranking, freezes `season_summary.v1`, binds the Closure Marker to the exact new Saved Revision, advances the clean Working Draft/Branch head and marks the Run `completed` in one transaction with retry/audit identity and rollback proof | Register future authoritative season-scoped statistic components and implement the ordinary season-0–48 transition layers (development/recovery, policy activation, resets, lifecycle/prospects, Week-1 Official Ranking/public state) |
