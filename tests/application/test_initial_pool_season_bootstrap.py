...
def services(
    ...
    bootstrap = InitialPoolSeasonBootstrapService(
        initial_pool_service=pool,
        active_players_path=tmp_path / "season_active_players.json",
    )
...
def test_dry_run_does_not_persist(tmp_path) -> None:
    ...
    result = bootstrap.bootstrap_from_initial_pool(
        season="2000/2001", seed=12345, dry_run=True
    )
...
def test_persist_preserves_provenance_and_zero_points(tmp_path) -> None:
    ...
    created = pool.create_custom_player(
        CustomInitialPoolPlayerCreate.model_validate(
            custom_api_payload("CUST-2000-AAA-BOOT")
        )
    )

    result = bootstrap.bootstrap_from_initial_pool(
        season="2000/2001", seed=222, dry_run=False
    )
    persisted = bootstrap.get_active_players(season="2000/2001")

    assert (
        persisted.summary.total_active_players
        == result.summary.total_active_players
        == 9
    )
...
@pytest.mark.parametrize(
    "birth_week,birth_year,expected_age",
    [(20, 1975, 25), (37, 1975, 25), (50, 1975, 24)],
)
def test_bootstrap_age_years_and_weeks_follow_birth_position(
    tmp_path, birth_week, birth_year, expected_age
):
    pool, bootstrap = services(tmp_path)
    from beta_engine.domain.players.initial_pool import CustomInitialPoolPlayerCreate

    payload = custom_api_payload(f"CUST-{birth_week}") | {
        "birth_year": birth_year,
        "birth_year_week": birth_week,
    }
    pool.create_custom_player(CustomInitialPoolPlayerCreate.model_validate(payload))
    player = bootstrap.bootstrap_from_initial_pool(
        season="2000/2001", dry_run=True
    ).players[0]
    assert player.age_years_at_season_start == expected_age
    assert player.age_weeks_at_season_start == completed_weeks_at_calendar_position(
        birth_year=birth_year,
        birth_year_week=birth_week,
        calendar_year=2000,
        year_week=37,
...
def test_duplicate_source_player_ids_rejected(tmp_path) -> None:
    pool, bootstrap = services(tmp_path)
    result = pool.generate_pool(
        season="2000/2001", seed=7, target_pool_size=2, dry_run=True
    )
    duplicate = result.players[0].model_dump(mode="json")
    (tmp_path / "pool.json").write_text(
        json.dumps({"players": [duplicate, duplicate], "audit_events": []}),
        encoding="utf-8",
    )
...
def test_overwrite_safety_and_season_isolation(tmp_path) -> None:
    ...
    bootstrap.bootstrap_from_initial_pool(
        season="2001/2002", source_season="2001/2002", seed=2, dry_run=False
    )
    with pytest.raises(ValueError, match="already exist"):
        bootstrap.bootstrap_from_initial_pool(season="2000/2001", seed=3, dry_run=False)

    overwritten = bootstrap.bootstrap_from_initial_pool(
        season="2000/2001", seed=3, dry_run=False, overwrite_existing=True
    )
    assert overwritten.metadata.bootstrap_seed == 3
    assert (
        bootstrap.get_active_players(season="2001/2002").summary.total_active_players
        == 3
    )
...
def test_fingerprint_determinism(tmp_path) -> None:
    ...
    first = bootstrap.bootstrap_from_initial_pool(
        season="2000/2001", seed=99, dry_run=True
    )
    second = bootstrap.bootstrap_from_initial_pool(
        season="2000/2001", seed=99, dry_run=True
    )
    third = bootstrap.bootstrap_from_initial_pool(
        season="2000/2001", seed=100, dry_run=True
    )

    assert first.model_dump(mode="json") == second.model_dump(mode="json")
    assert first.metadata.bootstrap_fingerprint != third.metadata.bootstrap_fingerprint
    assert (
        first.players[0].bootstrap_fingerprint != third.players[0].bootstrap_fingerprint
    )
...
def test_active_players_registry_loads_legacy_season_mapping(tmp_path) -> None:
    ...
    preview = bootstrap.bootstrap_from_initial_pool(
        season="2000/2001", seed=1, dry_run=True
    )
    (tmp_path / "season_active_players.json").write_text(
        json.dumps({"2000/2001": [preview.players[0].model_dump(mode="json")]}),
        encoding="utf-8",
    )

    response = bootstrap.get_active_players(season="2000/2001")
    assert response.summary.total_active_players == 1
    assert SeasonActivePlayersRegistry.model_validate(
        json.loads((tmp_path / "season_active_players.json").read_text())
    )
