...
describe('Module 17 pages through routes', () => {
  ...
  it('renders Tour & Seasons hub and shell routes while keeping operational routes available', async () => {
    ...
    expect(await screen.findByRole('heading', { level: 2, name: 'Concrete Season' })).toBeInTheDocument()
    ...
    expect(screen.getByText('This page is read-only. Concrete season editor, build from template, and compare/apply workflows are planned.')).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /build|apply|generate|save|create|update|delete/i })).not.toBeInTheDocument()
