...
def player(pid: str, *, entered: int = 1, retired: bool = False) -> OfficialRankingPlayer:
    return OfficialRankingPlayer(player_id=pid, tie_break_token=pid,
                                 tour_entry_week=week(entered), retired=retired)


def result(pid: str = "a", *, event: str = "e", completed: int = 2,
           published: int = 3, points: int = 100, **kwargs) -> OfficialRankingResult:
    return OfficialRankingResult(edition_id=event, player_id=pid, source_fingerprint=event,
                                 completed_week=week(completed), first_publication_week=week(published),
                                 main_points=points, **kwargs)


def calculate(n: int, *, players=None, results=(), previous=None, best_n=15):
    return calculate_official_ranking(run_id="run", branch_id="branch", week=week(n),
        policy=OfficialRankingPolicy(policy_id=f"best-{best_n}", best_n=best_n),
        players=players if players is not None else (player("a"),),
        results=results, previous=previous)
...
def test_season_policy_inherits_effective_previous_value_and_preserves_override():
    ...
    overridden = propose_next_season_policy(policy_id="override", previous_effective=changed, best_n_override=20)
...
def test_unranked_is_excluded_but_abandoned_terminal_result_is_eligible():
    awards = (result(event="unranked", ranked=False),
...
def test_point_profile_beats_token_and_result_count():
    awards = (result("a", event="a1", points=50), result("a", event="a2", points=50),
              result("b", points=100))
...
def test_newer_results_break_identical_point_profiles():
    ...
    assert calculate(4, players=(player("a"), player("b")), results=awards).rows[0].player_id == "b"


def test_previous_zero_player_precedes_new_entrant_even_with_worse_token():
    previous = calculate(2, players=(player("z"),))
    current = calculate(3, players=(player("a", entered=2), player("z")), previous=previous)
...
def test_previous_order_breaks_equal_nonzero_profiles():
    ...
    current = calculate(4, players=(player("a"), player("b")),
                        results=(result("a"), result("b")), previous=previous)
...
@pytest.mark.parametrize("field,value", [("run_id", "other"), ("branch_id", "other"), ("week", week(1))])
...
def test_duplicate_results_unknown_players_and_duplicate_tokens_fail_closed():
    ...
    with pytest.raises(ValueError, match="tokens must be unique"):
        calculate(3, players=(player("a"), player("b").model_copy(update={"tie_break_token": "a"})))


@pytest.mark.parametrize("kwargs", [{"published": 2}, {"points": -1}, {"points": True},
                                    {"validity_weeks": 0}, {"terminal_status": "suspended"}])
