...
describe('AdminRunBranchesPage', () => {
  ...
  it('renders scalar success and idempotent safety messages without another request', async () => {
    api.simulateNextMatchOnBranch.mockResolvedValueOnce(simulationSuccess(true)); renderWithRoute(<AdminRunBranchesPage />, '/admin/runs/run-a/branches'); await userEvent.click((await screen.findAllByRole('button', { name: 'Simulate Next Match' }))[0]); await userEvent.type(screen.getByLabelText('Simulation audit reason'), 'advance'); await userEvent.click(screen.getByLabelText('Confirm simulation')); await userEvent.click(screen.getByRole('button', { name: 'Confirm Simulate Next Match' }))
    const result = await screen.findByLabelText('Next Match result'); expect(result).toHaveTextContent('No duplicate match was simulated'); expect(result).toHaveTextContent('official Branch pointer was not changed'); expect(result).toHaveTextContent('cp-next'); expect(result).toHaveTextContent('simulation-fingerprint'); expect(result).not.toHaveTextContent('[object Object]'); expect(screen.getByLabelText('Confirm simulation')).not.toBeChecked(); expect(api.simulateNextMatchOnBranch).toHaveBeenCalledTimes(1)
  })
})
