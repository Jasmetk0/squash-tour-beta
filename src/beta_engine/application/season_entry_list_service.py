...
MAX_RUNTIME_PLAYER_AGE = 46  # legacy entry adapter clamp, not lifecycle policy

EntryListDecision = Literal[
    "accepted_main_draw",
...
class EntryListMetadata(BaseModel):
    ...
    ranking_basis: str = (
        "current zero-points bootstrap; future official ranking not integrated"
    )
...
class SeasonEntryListsRegistry(BaseModel):
    entry_lists_by_event_id: dict[str, SeasonEventEntryList] = Field(
...
class SeasonEntryListService:
    ...
    def generate_entry_list(
        self, *, event_id: str, request: EntryListGenerateRequest
        ...
        if not request.dry_run and exists and not request.overwrite_existing:
            raise ValueError(
                f"Entry list already exists for event '{event_id}'. Set overwrite_existing=true to replace only that event."
            )
        ...
        if event.ranking_status.value == "ranked" and not event.points_table_complete:
            ...
            raise ValueError(
                f"Ranked Edition has an incomplete points table; ranking-dependent entries are blocked. Missing required stages: {missing}"
            )
        active_response = self.active_players_service.get_active_players(
            season=str(event.season)
        )
        active_players = sorted(
            active_response.players, key=lambda player: player.player_id
        )
        if not active_players:
            raise ValueError(
                f"No active season players found for season '{event.season}'. Bootstrap active players before generating entries."
            )

        countries_by_code = {
            country.code: country for country in self.countries_service.list_countries()
        }
        ...
        engine = EntryEngine(
            ...
            tuning=load_entry_tuning_config(self.entry_tuning_path),
        )
        event_rng = engine.rng.branch(
            SeedScope.WEEK,
            self._season_seed_component(event.season),
            event.week,
            event.event_id,
        )
        ...
        for player in players:
            ...
            if country is None:
                ...
                continue
            decisions_by_player[player.player_id] = engine.decide_entry(
                player=player,
        ...
        main_candidates = [
            d for d in decisions_by_player.values() if d.target == EntryTarget.MAIN
        ]
        qualification_candidates = [
            d
            for d in decisions_by_player.values()
            if d.target == EntryTarget.QUALIFICATION
        ]
        ranked_main = self._sort_decisions(main_candidates, active_by_id)
        ranked_qualification = self._sort_decisions(
            qualification_candidates, active_by_id
        )
        ...
        for decision in ranked_main[:direct_slots]:
            entries.append(
                self._entry_from_decision(
                    ...
                    "accepted_main_draw",
                    priority,
                    reason="direct main draw acceptance",
                )
            )
        ...
        for decision in ranked_qualification[:qualification_slots]:
            ...
            entries.append(
                self._entry_from_decision(
                    ...
                    "accepted_qualification",
                    priority,
                    reason="qualification acceptance",
                )
            )
            used_players.add(decision.player_id)
            priority += 1
        ...
        remaining_entered = [
            d
            for d in self._sort_decisions(
                [
                    *ranked_main[direct_slots:],
                    *ranked_qualification[qualification_slots:],
                ],
                active_by_id,
            )
            if d.player_id not in used_players
        ]
        for decision in remaining_entered[: request.max_alternates]:
            entries.append(
                self._entry_from_decision(
                    ...
                    "alternate",
                    priority,
                    reason="waitlist alternate",
                )
            )
            used_players.add(decision.player_id)
            priority += 1
        if request.include_not_entered:
            for decision in self._sort_decisions(
                list(decisions_by_player.values()), active_by_id
            ):
                if decision.player_id in used_players:
                    continue
                decision_name = (
                ...
                reason = (
                    ...
                    else "entry roll did not enter tournament"
                )
                entries.append(
                    self._entry_from_decision(
                        decision,
        ...
        event_fp = (
            ...
            or self._fingerprint(event.model_dump(mode="json"))
        )
        validation_warnings, validation_errors = self.validate_event_entry_constraints(
            event=event,
            entries=entries,
            existing_entry_lists=registry.entry_lists_by_event_id,
        )
        warnings.extend(validation_warnings)
        summary = self._summary(
            active_players=active_players,
            considered=len(decisions_by_player),
            entries=entries,
            warnings=warnings,
        ...
        if not request.dry_run:
            if validation_errors:
                ...
                raise ValueError(
                    f"Entry list validation failed: {first.code}: {first.message}"
                )
        ...
        return SeasonEventEntryListResult(
            ...
            validation_warnings=warnings,
            validation_errors=validation_errors,
            entry_list_exists=exists or not request.dry_run,
        )

    def validate_event_entry_constraints(
        self,
        *,
        event: SeasonCalendarEvent,
        entries: list[SeasonEventEntry],
        existing_entry_lists: dict[str, SeasonEventEntryList],
    ) -> tuple[list[EntryListValidationIssue], list[EntryListValidationIssue]]:
        warnings: list[EntryListValidationIssue] = []
        ...
        for existing in existing_entry_lists.values():
            if existing.event_id == event.event_id or existing.season != str(
                event.season
            ):
                continue
            ...
            if calendar is not None:
                matched = next(
                    (
                        ...
                        if item.event_id == existing.event_id
                    ),
                    None,
                )
            ...
            for player_id in sorted(accepted & existing_accepted):
                errors.append(
                    self._issue(
                        "error",
                        "player_week_overlap",
                        f"player '{player_id}' is already accepted into overlapping event '{existing.event_id}'",
                        event_id=event.event_id,
                        player_id=player_id,
                        field="season_week",
                    )
        ...
        if not accepted:
            warnings.append(
                self._issue(
                    "warning",
                    "no_accepted_players",
                    "entry list has no accepted main draw or qualification players",
                    event_id=event.event_id,
                )
            )
    ...
    def _find_event(self, event_id: str) -> SeasonCalendarEvent:
        registry = (
            self.calendar_service._load_registry()
        )  # intentional application-layer registry reuse
        if not registry.calendars_by_season:
            raise ValueError(
                "No persisted season calendar exists. Persist a season calendar before generating entries."
            )
    ...
    def _adapt_player(player: SeasonActivePlayer) -> Player:
        ...
        return Player(
            ...
            age=max(
                MIN_RUNTIME_PLAYER_AGE,
                min(MAX_RUNTIME_PLAYER_AGE, player.age_years_at_season_start),
            ),
    ...
    def _template_from_event_snapshot(event: SeasonCalendarEvent) -> TournamentTemplate:
        ...
        payload: dict[str, Any] = {
            **snapshot,
            ...
            "byes": event.byes,
            "lucky_loser_rules": snapshot.get("lucky_loser_rules")
            or LuckyLoserRules(enabled=True, max_spots=0).model_dump(mode="json"),
            "point_distribution_ref": event.point_distribution_ref
            or snapshot.get("point_distribution_ref"),
            "point_distribution": event.point_distribution
            or snapshot.get("point_distribution"),
            "event_duration_days": snapshot.get("event_duration_days") or 1,
            "qualification_duration_days": snapshot.get("qualification_duration_days")
            or 0,
        ...
        if (
            payload.get("point_distribution_ref") is None
            and payload.get("point_distribution") is None
        ):
    ...
    @staticmethod
    def _sort_decisions(
        decisions: list[EntryDecision], players_by_id: dict[str, SeasonActivePlayer]
    ) -> list[EntryDecision]:
        return sorted(
            decisions,
            key=lambda decision: (
                -players_by_id[decision.player_id].ranking_points,
    ...
    def _entry_from_decision(
        ...
        player: SeasonActivePlayer,
        decision_name: EntryListDecision,
        priority: int,
        *,
        reason: str,
    ) -> SeasonEventEntry:
        ...
        return SeasonEventEntry(
            ...
            seed_candidate_rank=priority
            if decision_name == "accepted_main_draw"
            else None,
    ...
    def _summary(
        *,
        active_players: list[SeasonActivePlayer],
        considered: int,
        entries: list[SeasonEventEntry],
        warnings: list[EntryListValidationIssue],
        errors: list[EntryListValidationIssue],
        ...
        entered = [
            ...
            for entry in entries
            if entry.decision
            in {"accepted_main_draw", "accepted_qualification", "alternate"}
        ]
        return EntryListSummary(
            total_active_players=len(active_players),
            considered_players=considered,
            entered_players=len(entered),
            main_draw_acceptances=sum(
                1 for entry in entries if entry.decision == "accepted_main_draw"
            ),
            qualification_acceptances=sum(
                1 for entry in entries if entry.decision == "accepted_qualification"
            ),
            alternates=sum(1 for entry in entries if entry.decision == "alternate"),
            rejected_or_not_entered=sum(
                1 for entry in entries if entry.decision in {"rejected", "not_entered"}
            ),
            countries_represented=len({entry.country_code for entry in entered}),
            average_entry_probability=round(
                sum(entry.entry_probability for entry in entries) / len(entries), 6
            )
            if entries
            else 0.0,
            average_quality_score=round(
                sum(entry.quality_score for entry in entries) / len(entries), 6
            )
            if entries
            else 0.0,
    ...
    def _load_registry(self) -> SeasonEntryListsRegistry:
        ...
        return SeasonEntryListsRegistry.model_validate(
            json.loads(self.entry_lists_path.read_text(encoding="utf-8"))
        )
    ...
    def _save_registry(self, registry: SeasonEntryListsRegistry) -> None:
        ...
        tmp_path = self.entry_lists_path.with_suffix(
            f"{self.entry_lists_path.suffix}.tmp"
        )
        tmp_path.write_text(
    ...
    def _issue(
        ...
        *,
        ...
        return EntryListValidationIssue(
            severity=severity,
            code=code,
            message=message,
            event_id=event_id,
