...
- Active pre-alpha gameplans now materialize `Risk / Tempo / Court Positioning / Variation`, player-specific familiarity and imperfect opponent estimates before the match. Rally-by-rally AI may deliberately stick or adapt, while the plan acts causally through hidden control, pace, closure pressure and workload rather than a direct point bonus. Exact weights remain the replaceable `pre_alpha_gameplan_v1` calibration; see `docs/ACTIVE_GAMEPLAN_V1.md`.
- The pre-alpha rules resolver now separates terminal facts from `No Let / Yes Let / Stroke`, preserves score/server/service box on replay, and accounts for physical work and external-delay recovery exactly once. Incident rates remain provisional; see `docs/RALLY_RULES_RESOLVER_V1.md`.
