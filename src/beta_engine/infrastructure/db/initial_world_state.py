...
def get_initial_world(session: Session, *, run_id: str, branch_id: str) -> InitialWorldState | None:
    row = session.get(InitialWorldStateModel, (run_id, branch_id))
    if row is None:
        return None
    state = InitialWorldState.model_validate_json(row.payload_json)
    if (state.run_id, state.branch_id, state.fingerprint) != (run_id, branch_id, row.fingerprint):
...
def put_initial_world(session: Session, state: InitialWorldState) -> InitialWorldState:
    ...
    session.add(InitialWorldStateModel(run_id=state.run_id, branch_id=state.branch_id,
        fingerprint=state.fingerprint, payload_json=state.model_dump_json()))
    session.flush()
    installed = get_initial_world(session, run_id=state.run_id, branch_id=state.branch_id)
    if installed is None:
        raise ValueError("Initial-world write could not be verified")
    return installed


def capture_saved_initial_world(session: Session, payload: dict, *, run_id: str, branch_id: str) -> None:
    state = get_initial_world(session, run_id=run_id, branch_id=branch_id)
    content = payload["content"]
    if state is not None:
        content[INITIAL_WORLD_COMPONENT_KEY] = {"fingerprint": state.fingerprint, "state": state.model_dump(mode="json")}


def load_saved_initial_world(payload: dict, *, run_id: str, branch_id: str) -> InitialWorldState | None:
    ...
    if (state.run_id, state.branch_id, state.fingerprint) != (run_id, branch_id, component["fingerprint"]):
        raise ValueError("Saved initial-world identity or fingerprint mismatch")
    return state


def restore_saved_initial_world(session: Session, *, current_payload: dict, target_payload: dict,
                                run_id: str, branch_id: str) -> None:
    expected = load_saved_initial_world(current_payload, run_id=run_id, branch_id=branch_id)
    target = load_saved_initial_world(target_payload, run_id=run_id, branch_id=branch_id)
    live = get_initial_world(session, run_id=run_id, branch_id=branch_id)
    if (live.fingerprint if live else None) != (expected.fingerprint if expected else None):
    ...
    if target is not None:
        session.add(InitialWorldStateModel(run_id=run_id, branch_id=branch_id,
            fingerprint=target.fingerprint, payload_json=target.model_dump_json()))
        session.flush()
