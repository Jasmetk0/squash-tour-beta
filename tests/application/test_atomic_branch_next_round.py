...
def _legacy_snapshot(repository, run_id):
    with repository._session_factory() as session:
        return {model.__tablename__: _rows(session, model, run_id=run_id) for model in DURABLE_MODELS[:6]}
...
def test_legacy_next_round_and_branch_next_round_are_durably_equivalent(tmp_path):
    repository, service, branch_a, head_a = _setup(tmp_path)
    branch_b, head_b = _fork(repository, service, branch_a, head_a)
    service.simulate_next_round(run_id="source")
    service.simulate_next_round_on_branch_atomically(
        BranchSimulateNextRoundCommand("source", branch_b, head_b, "round-b", "equivalence", True))
...
def test_non_official_branch_next_round_is_isolated(tmp_path):
    repository, service, branch_a, head_a = _setup(tmp_path)
    branch_b, head_b = _fork(repository, service, branch_a, head_a)
    ...
    result = service.simulate_next_round_on_branch_atomically(
        BranchSimulateNextRoundCommand("source", branch_b, head_b, "isolated-round", "isolation", True))
...
def test_next_round_contract_and_journal_first_replay(tmp_path):
    repository, service, branch_id, head = _setup(tmp_path)
    command = BranchSimulateNextRoundCommand("source", branch_id, head, "next-round-1", "test", True)
...
def test_cross_action_different_commands_cannot_advance_same_head(tmp_path):
    ...
    with pytest.raises(BranchSimulationConflictError):
        service.simulate_next_round_on_branch_atomically(
            BranchSimulateNextRoundCommand("source", branch_id, head, "round-command", "cross action", True))
...
def test_command_id_cannot_be_reused_across_actions(tmp_path):
    ...
    with pytest.raises(BranchSimulationIdempotencyConflictError):
        service.simulate_next_round_on_branch_atomically(BranchSimulateNextRoundCommand("source", branch_id, head, "same-id", "test", True))


def test_reviewed_state_fingerprint_mismatch_rolls_back(tmp_path):
    repository, service, branch_id, head = _setup(tmp_path)
    run, reviewed = service._load_run_context(run_id="source")
    step = service._build_orchestrator(season=run.season, seed=run.seed, run_info=run).simulate_next_round(state=reviewed)
    ...
    before = _durable_snapshot(repository)
    with pytest.raises(BranchSimulationConflictError):
        repository.simulate_next_round_on_branch_atomically(
            BranchSimulateNextRoundCommand("source", branch_id, head, "mismatch", "test", True),
            step=step, reviewed_pre_state_fingerprint=reviewed_hash)
...
def test_next_round_failure_rolls_back_complete_durable_snapshot(tmp_path, monkeypatch, failure):
    repository, service, branch_id, head = _setup(tmp_path)
    before = _durable_snapshot(repository)
    if failure == "legacy":
    ...
    with pytest.raises(RuntimeError):
        service.simulate_next_round_on_branch_atomically(
            BranchSimulateNextRoundCommand("source", branch_id, head, f"fail-{failure}", "fault injection", True))
