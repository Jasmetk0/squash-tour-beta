...
- **Fair-rest scheduler depth started:** automatic V2 proposals now implement the bounded Master §13.4 feeder-finish ordering rule: a dependent path whose previous match finished later in stored Match Day order is kept later before structural tie-breaks. Remaining scheduler depth is minimum-rest maximization, recovery-gap balancing, explicit Q/LL protection, previous-week carryover, deterministic tie-break authority and later minimal-range preservation/reflow. Numeric daily capacity, precise times, courts and travel/acclimatization mathematics remain open/deferred as specified by Master.
