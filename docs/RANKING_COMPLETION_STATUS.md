...
| Season Closing Ranking | `season_closing_ranking.v1` calculates the archived Week-61 snapshot under the outgoing policy; seasons 0–48 persist Week-61 sources against next Season Week 1, while 2049/50 uses Closing-only owned source v6 with a post-final boundary ordinal and no fictional Week 1. Final 2049/50 has an atomic closure command. Ordinary rollover has `season_transition_configuration.v1`, cross-season sporting/lifecycle/Week-1 Ranking staging and a real atomic writer: one transaction stages Closing Ranking + Season Summary/Marker, persists W1 sporting/lifecycle/ranking, publishes the Official Ranking, advances world state, emits the Season event and stores a complete Saved Revision + audit trail; preflight fingerprints Closing/configuration/sporting/lifecycle/Ranking candidates and only requires the still-missing prospect bridge when target W1 has unbridged prospects | Register future authoritative season-scoped statistic components and implement the prospect/Tour-entry canonical sporting-profile bridge |
