...
class AuthoritativeWeekTransitionRunner:
    ...
    def execute(self, command: AuthoritativeWeekTransitionCommand, *, expected_ranking_fingerprint=None):
        with self.factory.begin() as session:
            session.execute(text("BEGIN IMMEDIATE"))
            result = transition_in_transaction(session, self.awards, command)
            if expected_ranking_fingerprint is not None and result.official_ranking_fingerprint != expected_ranking_fingerprint:
...
def transition_in_transaction(session: Session, awards, command):
    ...
    command = AuthoritativeWeekTransitionCommand.model_validate_json(command.model_dump_json())
    ...
    if run is None or branch is None or draft is None or branch.run_id != command.run_id:
    ...
    if branch.saved_head_revision_id != command.base_revision_id or draft.base_revision_id != command.base_revision_id:
    ...
    if (authority.completed_week, authority.target_week) != (command.completed_week, command.target_week):
        raise ValueError("Ranking transition authority boundary differs")

    candidates = OfficialRankingCandidateStore(session).history(
        run_id=command.run_id, branch_id=command.branch_id)
    predecessor = next((value for value in candidates if value.week == command.completed_week), None)
    ...
    if world is None:
        ...
        session.add(PublishedOfficialRankingModel(
            run_id=command.run_id, branch_id=command.branch_id, week_ordinal=0,
            snapshot_fingerprint=predecessor.fingerprint,
            payload_json=predecessor.model_dump_json(),
        ))
        world = AuthoritativeWorldStateModel(
            run_id=command.run_id, branch_id=command.branch_id, current_ordinal=0,
            ranking_fingerprint=predecessor.fingerprint,
        )
        session.add(world)
        session.flush()
    elif world.current_ordinal != command.completed_week.ordinal or world.ranking_fingerprint != predecessor.fingerprint:
        raise ValueError("Authoritative world predecessor state differs")

    ranking_command = RankingWeekCommand(
        command_id=f"{command.command_id}:ranking", context=RankingTransitionContext(
            run_id=command.run_id, branch_id=command.branch_id,
            completed_week=authority.completed_week, target_week=authority.target_week,
            policy=authority.policy, players=authority.players, discipline="stored_zeros",
        ), tournaments=command.tournaments, corrections=command.corrections,
        zero_versions=command.zero_versions, audit=command.audit,
        authority_fingerprint=authority.fingerprint,
    )
    snapshot = stage_ranking_week_command(session, awards, ranking_command)
    _fault_injection_point("after_ranking_staging")
    if session.get(PublishedOfficialRankingModel, (command.run_id, command.branch_id, command.target_week.ordinal)) is not None:
    ...
    session.add(PublishedOfficialRankingModel(
        ...
        snapshot_fingerprint=snapshot.fingerprint, payload_json=snapshot.model_dump_json(),
    ))
    ...
    result = AuthoritativeWeekTransitionResult(
        run_id=command.run_id, branch_id=command.branch_id, command_id=command.command_id,
        completed_week=command.completed_week, target_week=command.target_week,
        official_ranking_fingerprint=snapshot.fingerprint,
    )
    session.add(AuthoritativeWeekTransitionReceiptModel(
        run_id=command.run_id, branch_id=command.branch_id, command_id=command.command_id,
        request_fingerprint=command.fingerprint, request_payload_json=command.canonical_request_json,
