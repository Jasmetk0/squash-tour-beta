...
from beta_engine.infrastructure.db import (
    ...
    BranchSimulateNextTournamentCommand, BranchSimulationConflictError,
...
def _setup(tmp_path):
    engine = create_sqlite_engine(DatabaseSettings(url=f"sqlite:///{tmp_path / 'next-tournament.db'}"))
...
def test_legacy_next_tournament_and_branch_next_tournament_are_durably_equivalent(tmp_path):
    repository, service, branch_a, head_a = _setup(tmp_path)
    branch_b, head_b = _fork(repository, service, branch_a, head_a)
    service.simulate_next_tournament(run_id="source")
    service.simulate_next_tournament_on_branch_atomically(
        BranchSimulateNextTournamentCommand("source", branch_b, head_b, "tournament-b", "equivalence", True))
    ...
    with repository._session_factory() as session:
        ...
        assert session.execute(select(RankingSnapshotModel).where(RankingSnapshotModel.run_id == "legacy-b", RankingSnapshotModel.snapshot_kind == "week")).scalars().all().__len__() == 0


def test_active_tournament_is_finalized_without_simulating_another_event(tmp_path):
    repository, service, branch_a, head_a = _setup(tmp_path)
    branch_b, head_b = _fork(repository, service, branch_a, head_a)
    ...
    service.simulate_next_tournament(run_id="source")
    tournament = service.simulate_next_tournament_on_branch_atomically(
        BranchSimulateNextTournamentCommand(
            "source", branch_b, branch_match.new_head_checkpoint_id,
            "active-tournament", "finalize active tournament", True,
        )
    )
    assert _legacy_snapshot(repository, "source") == {
        table: [{**row, "run_id": "source"} for row in rows]
    ...
    with repository._session_factory() as session:
        ...
        assert len(events) == len(inputs) == len(metadata) == len({row.event_id for row in events}) == 1
        assert events[0].event_id == "ev_2027_w01_malaysia_major"
        assert session.execute(select(BranchCheckpointModel).where(BranchCheckpointModel.parent_checkpoint_id == branch_match.new_head_checkpoint_id)).scalars().one().checkpoint_id == tournament.new_head_checkpoint_id
        assert session.execute(select(BranchSimulationCommandModel).where(BranchSimulationCommandModel.command_id == "active-tournament")).scalars().one().resulting_head_checkpoint_id == tournament.new_head_checkpoint_id
...
def test_final_active_tournament_is_persisted_without_weekly_result(tmp_path):
    ...
    legacy_step = service.simulate_next_tournament(run_id="source")
    assert legacy_step.weekly_result is None
    result = service.simulate_next_tournament_on_branch_atomically(
        BranchSimulateNextTournamentCommand("source", branch_b, branch_match.new_head_checkpoint_id, "final-tournament", "finalize final", True)
    )
    assert _legacy_snapshot(repository, "source") == {
        table: [{**row, "run_id": "source"} for row in rows]
        for table, rows in _legacy_snapshot(repository, "legacy-b").items()
...
def test_non_official_branch_next_tournament_is_isolated(tmp_path):
    repository, service, branch_a, head_a = _setup(tmp_path)
    branch_b, head_b = _fork(repository, service, branch_a, head_a)
    ...
    result = service.simulate_next_tournament_on_branch_atomically(
        BranchSimulateNextTournamentCommand("source", branch_b, head_b, "isolated-tournament", "isolation", True))
...
def test_next_tournament_contract_and_journal_first_replay(tmp_path):
    repository, service, branch_id, head = _setup(tmp_path)
    command = BranchSimulateNextTournamentCommand("source", branch_id, head, "next-tournament-1", "test", True)
    result = service.simulate_next_tournament_on_branch_atomically(command)
    with repository._session_factory() as session:
        ...
        assert journal.action_kind == "simulate_next_tournament"
        assert checkpoint.command_kind == "simulate_next_tournament_branch"
        assert checkpoint.command_boundary == "after_branch_next_tournament_persisted"
    ...
    replay = service.simulate_next_tournament_on_branch_atomically(command)
...
def test_cross_action_different_commands_cannot_advance_same_head(tmp_path):
    ...
    assert isinstance(week, BranchSimulateNextWeekResult)
    after_one = _durable_snapshot(repository)
    with pytest.raises(BranchSimulationConflictError):
        service.simulate_next_tournament_on_branch_atomically(
            BranchSimulateNextTournamentCommand("source", branch_id, head, "tournament-command", "cross action", True))
...
def test_command_id_cannot_be_reused_across_actions(tmp_path):
    ...
    with pytest.raises(BranchSimulationIdempotencyConflictError):
        service.simulate_next_tournament_on_branch_atomically(BranchSimulateNextTournamentCommand("source", branch_id, head, "same-id", "test", True))


def test_reviewed_state_fingerprint_mismatch_rolls_back(tmp_path):
    repository, service, branch_id, head = _setup(tmp_path)
    run, reviewed = service._load_run_context(run_id="source")
    step = service._build_orchestrator(season=run.season, seed=run.seed, run_info=run).simulate_next_tournament(state=reviewed)
    ...
    before = _durable_snapshot(repository)
    with pytest.raises(BranchSimulationConflictError):
        repository.simulate_next_tournament_on_branch_atomically(
            BranchSimulateNextTournamentCommand("source", branch_id, head, "mismatch", "test", True),
...
@pytest.mark.parametrize("failure", ["legacy", "journal"])
def test_next_tournament_failure_rolls_back_complete_durable_snapshot(tmp_path, monkeypatch, failure):
    repository, service, branch_id, head = _setup(tmp_path)
    before = _durable_snapshot(repository)
    if failure == "legacy":
        ...
        needles = {"journal": "INSERT INTO branch_simulation_commands"}
    ...
    with pytest.raises(RuntimeError):
        service.simulate_next_tournament_on_branch_atomically(
            BranchSimulateNextTournamentCommand("source", branch_id, head, f"fail-{failure}", "fault injection", True))
