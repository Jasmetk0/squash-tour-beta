...
def test_rankings_accept_compact_labels(tmp_path: Path) -> None:
    with Server(tmp_path) as server:
        server.persist_ranked_players()
        status_long, long_body = call("GET", f"{server.base_url}/admin/rankings/2000%2F2001?table_type=ranking")
        status_compact, compact_body = call("GET", f"{server.base_url}/admin/rankings/2000%2F01?table_type=ranking")
