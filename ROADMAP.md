...
- Foundation hardened: one-event orchestration now returns structured QA/diagnostic reports with plan summaries, before/after artifact state, lifecycle transitions, stop reasons, safe rerun hints, mutation flags, and backend-produced next safe actions.
- Foundation added: Admin Seasons can preview week-level orchestration with a read-only Simulate One Season Week preflight report that reuses one-event dry-run reports for all persisted events in a season week.
- Foundation added: Admin Seasons can run a guarded mutating one-week simulation command that first executes week preflight, then runs selected events in deterministic backend order, optionally applies points, and optionally publishes one weekly ranking/race snapshot after event point applications complete.
- Foundation added: Admin Seasons can inspect read-only week run recovery / partial-run diagnostics for persisted season-week events, including artifact presence, point award/application status, snapshot status, blocker state, duplicate-point/overwrite risk, and backend-recommended safe next actions.
- Foundation added: Admin Seasons can inspect read-only season simulation readiness across all 61 season weeks, aggregating week recovery reports into season-level next-safe-action, blocker, completion, point-application, and snapshot-publication status.
- Foundation added: Admin Seasons can preview a selected season-week range with read-only range preflight, including skipped empty/complete weeks, runnable weeks, point/snapshot planning, blockers, first unsafe week, and recommended future run flags.
- Foundation added: Admin Seasons can run guarded mutating season-week ranges from range preflight, skipping empty/complete weeks, executing actionable weeks in backend deterministic order, and returning range-level execution reports.
- UI cleanup added: Admin Seasons is reorganized into workflow-oriented collapsible sections for local testing, with read-only/mutating/advanced/debug badges and no backend logic or API shape changes.
- Remaining limitations: no full run-season convenience command yet, rollback is not implemented, range run still uses file-backed artifacts, week readiness, season readiness, and range preflight are read-only, partial week/range runs must be inspected and rerun manually, one-event diagnostics do not replace domain validation, rolling 61-week ranking is not implemented, best-N ranking selection is not implemented, progression/regression is not implemented, annual intake/retirement is not implemented, cross-season movement is not implemented, ranking history analytics such as weeks at #1 and career high are not implemented, full Viewer player profiles are not implemented, Viewer player profile links may still be placeholders, prize money is not awarded, reapply/revert workflow is not implemented, withdrawals/lucky losers are not connected, advanced schedule-of-play is not implemented, Viewer tournament pages are not complete, and final database-backed tournament history persistence is not implemented.
