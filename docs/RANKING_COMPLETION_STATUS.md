...
| Season Closing Ranking | `season_closing_ranking.v1` calculates the archived Week-61 snapshot under the outgoing policy; seasons 0–48 persist Week-61 sources against next Season Week 1, while 2049/50 uses Closing-only owned source v6 with a post-final boundary ordinal and no fictional Week 1; resolver/staging and `ranking_revision_state.v6` preserve the archive. `season_summary.v1` plus the Closure Marker candidate form the step-3 closure package, and a guarded lifecycle kernel can mark a Run `completed` only from matching 2049/50 Week-61 closure evidence in the Branch-head Saved Revision | Register future authoritative season-scoped statistic components and create/bind the final closure Saved Revision inside the full atomic Season Transition command |
