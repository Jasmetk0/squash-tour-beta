...
## Materialized Branch fork: real Qualification-receipt Lucky Loser acceptance

PR-critical acceptance now proves the Lucky Loser vacancy/fill fork path from real authoritative Qualification receipts rather than mocked evidence. A canonical four-player Qualification bracket is executed through `AuthoritativeSlotMatchExecutor` across real Q semifinal and final Simulation Slots, producing persisted `SimulationEventGroupModel` result receipts. A later frozen Main vacancy resolves the LL order from those receipts, creates the chronological LL vacancy and fills it from the frozen candidate order.

The complete sporting + Simulation Slot Saved Revision is then remapped onto a target Branch and materialized. Target LL vacancy/fill revisions must own target Branch identity, Q terminal and candidate elimination fingerprints must equal the source->target result map, and exact retries of both restored LL commands must not append duplicate revisions.
