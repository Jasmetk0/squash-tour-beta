...
describe('Module 17 pages through routes', () => {
  ...
  it('renders Season Builder read-only source selection preview', async () => {
    ...
    expect(screen.getAllByText('preflight_fingerprint').length).toBeGreaterThan(0)
    expect(screen.getAllByText('reviewed_diff_id').length).toBeGreaterThan(0)
    expect(screen.getAllByText('pf_test_existing').length).toBeGreaterThan(0)
    expect(screen.getAllByText('rd_test_existing').length).toBeGreaterThan(0)
    expect(screen.getByText('Dry-run build command contract exists, but execution is disabled in this phase.')).toBeInTheDocument()
    expect(screen.getByText('audit_reason will be required before execution is enabled in a future phase.')).toBeInTheDocument()
    expect(screen.getByText('explicit_confirmation will be required before execution is enabled in a future phase.')).toBeInTheDocument()
    expect(screen.getByText('mutation_scope will be required before execution is enabled in a future phase.')).toBeInTheDocument()
    expect(screen.getAllByText('execution_enabled').length).toBeGreaterThan(0)
    expect(screen.getAllByText('explicit_confirmation_present').length).toBeGreaterThan(0)
    expect(screen.getByText('Raw disabled dry-run build contract JSON')).toBeInTheDocument()
