...
| Season Closing Ranking | `season_closing_ranking.v1` calculates the archived Week-61 snapshot under the outgoing policy; seasons 0–48 persist Week-61 sources against next Season Week 1, while 2049/50 uses Closing-only owned source v6 with a post-final boundary ordinal and no fictional Week 1. Final 2049/50 has an atomic closure command. Ordinary rollover has `season_transition_configuration.v1` plus cross-season sporting, existing-player lifecycle and Week-1 Official Ranking staging: W61 development/recovery uses the outgoing Development Policy, W1 installs the selected incoming policies, lifecycle applies canonical birthdays/retirement while refusing to omit unbridged prospects, and the Ranking candidate reuses canonical result/zero history plus Week-61 owned tournament sources without publication; preflight fingerprints configuration, sporting, lifecycle and Ranking candidates | Register future authoritative season-scoped statistic components and implement the prospect/Tour-entry sporting-profile bridge plus public-state/atomic rollover commit |
