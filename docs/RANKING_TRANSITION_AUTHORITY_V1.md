...
An explicit audited adoption stores an immutable Run/Branch-owned snapshot containing:
...
world-time projections are not yet available. Legacy season files are not silently
accepted as branch truth.
...
idempotent and conflicting replacements fail closed.
...
capture and validate them. V1 and V2 payloads remain readable and hash-preserving when
the new collection is absent. Viewer routes do not expose draft authority or candidates.
