...
| Packages/players | World package, owned InitialWorld, lifecycle, `player_sporting_week_states`, slot checkpoints, and branch-owned target-week prospect arrival evidence in Week Transition World Events | Canonical 57×0–200 sporting history and match-derived Form/Sharpness/Fatigue exist for the narrow slot slice; health and later pre-Tour→Tour entry AI remain open |
...
| Week execution | `infrastructure/db/authoritative_week_transition.py` owns one `BEGIN IMMEDIATE`; owned complete tournament result manifests can resolve completed-match counts; target-week 15-year-old prospect intake is fingerprint-bound and persisted into the branch-owned Week Transition World Event | Sporting development then between-week recovery, lifecycle, ranking/publication/event/receipt share the transaction; a missing/empty authoritative sporting context still fails closed, while broader health and future Tour-entry AI remain outside it |
...
- #738 is treated as a Wild Card provenance primitive, not the completed WC/RWC
  ...
  authority and Week 61 rollover remain fail-closed. Target-week Run prospects are no longer promoted into the ranking roster or rejected as an unowned source: the server freezes their exact pre-Tour snapshot/fingerprint into the branch-owned Week Transition World Event, and a changed source after preview invalidates confirmation.
