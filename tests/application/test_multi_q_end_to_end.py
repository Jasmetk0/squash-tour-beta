...
def test_four_q_sections_execute_into_main_and_close_with_additive_points(tmp_path):
    """Four canonical Q brackets feed Q1..Q4 into one eight-player Main."""

    root = tmp_path / "canonical-four-q"
    ...
    assert all(len(bracket.nodes) == 1 for bracket in draw.qualification_brackets)
    assert len(draw.main.nodes) == 7
    assert {
        placeholder for placeholder, _ in draw.main.qualifier_placeholder_slots
    } == {"Q1", "Q2", "Q3", "Q4"}
    session.commit()
    ...
    assert len(scheduled_group_ids) == 11
    ...
    with factory() as db:
        ...
        assert len(set(result.qualification_winner_ids)) == 4
        ...
        q_value = distribution["qualification_winner"]

        for player_id in result.qualification_winner_ids:
            player = result_by_player[player_id]
            award = awards_by_player[player_id]
            assert player.draw_type == "both"
            assert player.qualifier is True
            assert award.qualification_point_stage == "qualification_winner"
            assert award.qualification_points_awarded == q_value
            main_stage = award.point_stage or award.reached_stage
            assert award.ranking_points_awarded == (
                distribution[main_stage] + q_value
            )
            assert award.race_points_awarded == (
                distribution[main_stage] + q_value
            )

        q_losers = set(qualification_ids) - set(result.qualification_winner_ids)
        assert len(q_losers) == 4
        for player_id in q_losers:
            player = result_by_player[player_id]
            award = awards_by_player[player_id]
            assert player.draw_type == "qualification"
            assert player.reached_stage == "qualification_final"
