...
PR-critical coverage now spans a mixed source Draw Revision lineage (full redraw followed by frozen Reserve Wild Card repair), source-to-target Branch remap, exact target installation, a later third live Draw-Freeze revision, restore back to the materialized fork root, and retry of the restored RWC command. The target revision lineage remains target-owned, chronological and fingerprint-stable, while source and target revision fingerprints remain distinct.
