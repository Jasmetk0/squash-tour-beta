...
| 2 — one true week boundary (implemented first slice; expansion in progress) | One SQLite owner derives birthday/age-46 retirement and the Official roster from owned lifecycle state, publishes ranking, advances the clock, emits event/receipt and survives Save/Restore atomically | Development/recovery remain open blockers; prospects fail closed until an authoritative owned source is integrated; next prove repeated flow and public consumers |
