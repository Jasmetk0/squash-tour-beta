...
function setup({ branches = [official, readonly], states = [state(), state('readonly', 'cp-readonly')], checkpoints = [checkpoint(), checkpoint('cp-readonly', 'initial', 'readonly')] } = {}) { api.getRunContainer.mockResolvedValue(run); api.listRunBranches.mockResolvedValue({ run_branches: branches }); api.listBranchStates.mockResolvedValue({ branch_states: states }); api.listBranchCheckpoints.mockResolvedValue({ branch_checkpoints: checkpoints }); api.listSavedRevisionHistory.mockImplementation((runId: string, branchId: string) => Promise.resolve({ run_id: runId, branch_id: branchId, saved_head_revision_id: 'revision-head', saved_revisions: [] })); api.getSavedRevisionRecoveryActivity.mockImplementation((runId: string, branchId: string) => Promise.resolve({ run_id: runId, branch_id: branchId, saved_head_revision_id: 'revision-head', safety_checkpoints: [], audit_events: [] })); api.forkRunBranch.mockResolvedValue(success()); api.simulateNextMatchOnBranch.mockResolvedValue(simulationSuccess()); api.simulateNextRoundOnBranch.mockResolvedValue({ ...simulationSuccess(), simulation_result: { ...simulationSuccess().simulation_result, mode: 'simulate_next_round' } }); api.simulateNextWeekOnBranch.mockResolvedValue({ ...simulationSuccess(), simulation_result: { ...simulationSuccess().simulation_result, mode: 'simulate_next_week' } }); api.simulateNextTournamentOnBranch.mockResolvedValue({ ...simulationSuccess(), simulation_result: { ...simulationSuccess().simulation_result, mode: 'simulate_next_tournament' } }); api.simulateFullSeasonOnBranch.mockResolvedValue(fullSeasonSuccess()); api.simulateWorldTourFinalsOnBranch.mockResolvedValue(finalsSuccess()) }
...
const finalsSuccess = (replay = false) => ({ ...simulationSuccess(replay), new_head_checkpoint_id: 'cp-finals', current_event_sequence: 4, finals: { run_id: 'legacy-official', season: 2030, event_id: 'WTF-2030', already_simulated: false, qualification: { run_id: 'legacy-official', season: 2030, source_as_of_season: 2030, source_as_of_week: 12, qualification: { target_season: 2030, qualifier_count: 1, reserve_count: 0, qualified: [{ player_id: 'p1', race_rank: 1, race_points: 100, seed: 1 }], reserves: [], ineligible_race_entries: [] } }, result: { run_id: 'legacy-official', season: 2030, event_id: 'WTF-2030', source_as_of_season: 2030, source_as_of_week: 12, result: { event_id: 'WTF-2030', season: 2030, qualification: { target_season: 2030, qualifier_count: 1, reserve_count: 0, qualified: [], reserves: [], ineligible_race_entries: [] }, groups: [], knockout: [], placements: [{ player_id: 'p1', finish: 'champion' }] } } }, simulation_result: undefined })
...
describe('AdminRunBranchesPage', () => {
  it('reviews and submits World Tour Finals through only the Finals client', async () => {
    renderWithRoute(<AdminRunBranchesPage />, '/admin/runs/run-a/branches')
    const actions = await screen.findAllByRole('button', { name: 'Simulate World Tour Finals' }); expect(actions).toHaveLength(2); expect(actions[0]).toBeEnabled(); expect(actions[1]).toBeDisabled()
    await userEvent.click(actions[0]); expect(api.simulateWorldTourFinalsOnBranch).not.toHaveBeenCalled(); expect(screen.getByText(/persists the complete Finals event atomically/)).toBeInTheDocument()
    const commandId = screen.getByLabelText('Simulation command ID').textContent
    await userEvent.type(screen.getByLabelText('Simulation audit reason'), ' reviewed Finals '); await userEvent.click(screen.getByLabelText('Confirm simulation')); await userEvent.click(screen.getByRole('button', { name: 'Confirm Simulate World Tour Finals' }))
    await waitFor(() => expect(api.simulateWorldTourFinalsOnBranch).toHaveBeenCalledTimes(1))
    expect(api.simulateWorldTourFinalsOnBranch).toHaveBeenCalledWith('run-a', 'official', { expected_head_checkpoint_id: 'cp-initial', command_id: commandId, audit_reason: 'reviewed Finals', explicit_confirmation: true })
    expect(api.simulateNextMatchOnBranch).not.toHaveBeenCalled(); const result = await screen.findByLabelText('World Tour Finals result'); expect(result).toHaveTextContent('cp-finals'); expect(result).toHaveTextContent('WTF-2030'); expect(result).toHaveTextContent('2030 / 12 / EVENT-12 / 4')
  })
  it.each([
    ['nested Finals event', (response: ReturnType<typeof finalsSuccess>) => { response.finals.result.result.event_id = 'WRONG-EVENT' }],
    ['nested Finals season', (response: ReturnType<typeof finalsSuccess>) => { response.finals.result.result.season = 2029 }],
    ['qualification target season', (response: ReturnType<typeof finalsSuccess>) => { response.finals.qualification.qualification.target_season = 2029 }]
  ])('rejects a malformed %s identity without trusting or retrying the response', async (_caseName, corrupt) => {
    const malformed = finalsSuccess(); corrupt(malformed); api.simulateWorldTourFinalsOnBranch.mockResolvedValueOnce(malformed)
    renderWithRoute(<AdminRunBranchesPage />, '/admin/runs/run-a/branches'); await userEvent.click((await screen.findAllByRole('button', { name: 'Simulate World Tour Finals' }))[0]); await userEvent.type(screen.getByLabelText('Simulation audit reason'), 'review malformed Finals'); await userEvent.click(screen.getByLabelText('Confirm simulation')); await userEvent.click(screen.getByRole('button', { name: 'Confirm Simulate World Tour Finals' }))
    expect(await screen.findByText('Response contract error: the World Tour Finals response is missing or inconsistent.')).toBeInTheDocument()
    expect(screen.queryByLabelText('World Tour Finals result')).not.toBeInTheDocument(); expect(screen.queryByText('The official Branch pointer was not changed.')).not.toBeInTheDocument(); expect(screen.queryByText('cp-finals')).not.toBeInTheDocument(); expect(screen.getAllByText('cp-initial').length).toBeGreaterThan(0)
    await waitFor(() => { expect(api.listRunBranches.mock.calls.length).toBeGreaterThanOrEqual(2); expect(api.listBranchStates.mock.calls.length).toBeGreaterThanOrEqual(2); expect(api.listBranchCheckpoints.mock.calls.length).toBeGreaterThanOrEqual(2) })
    expect(api.simulateWorldTourFinalsOnBranch).toHaveBeenCalledTimes(1)
  })
