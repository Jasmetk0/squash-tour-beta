...
def test_simulate_next_completes_first_pending_and_is_replay_deterministic(
    ...
    assert a_completed.simulated_result.rally_log.schema_version == "match_rally_log.v6"
...
@pytest.mark.smoke
def test_post_draw_replacement_shortcut_fails_closed_without_mutation(tmp_path: Path) -> None:
    service, event_id = make_match_service(tmp_path)
    ...
    assert concrete_slot.player_id is not None
    ...
    with pytest.raises(
        ...
        service.record_late_replacement(
            event_id=event_id,
            withdrawn_player_id=concrete_slot.player_id,
        )

    assert service.get_late_replacements(event_id=event_id) == ()
    ...
    assert concrete_slot.player_id in {
        player_id
...
def test_post_draw_replacement_stays_locked_after_match_package(tmp_path: Path) -> None:
    service, event_id = make_match_service(tmp_path)
    ...
    service.generate_match_package(
        event_id=event_id,
        request=MatchPackageGenerateRequest(seed=1402, dry_run=False),
    )
