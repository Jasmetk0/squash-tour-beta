"""Atomic ordinary Season Transition writer for Season Week 61 -> next Season Week 1."""
...
class OrdinarySeasonTransitionCommand(FrozenInput):
    ...
    season_saved_revision_id: str = Field(min_length=1, max_length=128)
    ...
    return json.dumps(
        {
            ...
            "closing_ranking_fingerprint": closing_ranking_fingerprint,
            "season_summary_fingerprint": season_summary_fingerprint,
            "closure_marker_fingerprint": closure_marker_fingerprint,
...
def _retry_result(
    ...
    revision = session.get(BranchSavedRevisionModel, command.season_saved_revision_id)
    if revision is None:
        return None
    ...
    if (
        ...
        or revision.kind != SEASON_TRANSITION_SAVED_REVISION_KIND
        or branch is None
        or branch.saved_head_revision_id != command.season_saved_revision_id
        or audit is None
        or audit.audit_event_id != command.audit_event_id
        or audit.event_kind != SEASON_TRANSITION_AUDIT_EVENT_KIND
    ):
        raise ValueError("Season Transition revision identity is already in conflict")
    ...
    closure = load_saved_revision_season_closure(
        ...
        revision_id=command.season_saved_revision_id,
    )
    if closure is None:
        raise ValueError("Season Transition revision lost season-closure evidence")
    try:
    ...
    target = command.configuration.target_week
    ...
    closing = SeasonClosingRankingStore(session).get(
        run_id=command.run_id,
        branch_id=command.branch_id,
        season_index=command.configuration.completed_week.season_index,
    )
    event = session.get(
        AuthoritativeWorldEventModel,
    ...
    if (
        ...
        != audit_payload.get("official_ranking_fingerprint")
        or world.current_ordinal != target.ordinal
        or world.ranking_fingerprint != publication.snapshot_fingerprint
        or lifecycle.fingerprint != audit_payload.get("player_lifecycle_fingerprint")
        or sporting.fingerprint != audit_payload.get("player_sporting_fingerprint")
        or closing.fingerprint != audit_payload.get("closing_ranking_fingerprint")
        or event.event_kind != "season_transition_completed"
        or event.week_ordinal != target.ordinal
    ):
        raise ValueError("Completed Season Transition evidence is corrupt")

    marker = closure.parsed_marker
    summary = closure.parsed_summary
    if event.payload_json != _world_event_payload(
        command,
        ...
        raise ValueError("Completed Season Transition world event is corrupt")
    ...
    if (
        draft is None
        or draft.base_revision_id != command.season_saved_revision_id
        or draft.status != CLEAN_WORKING_DRAFT_STATUS
    ):
        raise ValueError("Season Transition retry has incoherent Working Draft")
...
def commit_ordinary_season_transition(
    ...
    *,
    ...
    """Commit one non-final W61 -> next-season W1 transition in the caller transaction."""
    ...
    if configuration.completed_week.season_index >= 49:
        raise ValueError("Ordinary Season Transition cannot open a season after 2049/50")
    if configuration.reset_catalog.component_ids:
        raise ValueError(
            "Season Transition reset catalog contains components without an atomic reset adapter"
        )
    ...
    if session.get(BranchSavedRevisionModel, command.season_saved_revision_id) is not None:
    ...
    snapshot = ranking.target_snapshot

    if fault_at == "after_staging":
        raise RuntimeError("fault after ordinary Season Transition staging")
    ...
    if (
        ...
        != configuration.predecessor_official_fingerprint
    ):
        raise ValueError("Season Transition authoritative Week-61 world head changed")
    if (
        session.get(
            PublishedOfficialRankingModel,
            (command.run_id, command.branch_id, configuration.target_week.ordinal),
        )
        is not None
    ):
    ...
    marker = bind_season_closure_marker(
        package.marker,
        final_saved_revision_id=command.season_saved_revision_id,
    )
    ...
    session.add(
        PublishedOfficialRankingModel(
            run_id=command.run_id,
            branch_id=command.branch_id,
            week_ordinal=configuration.target_week.ordinal,
            snapshot_fingerprint=snapshot.fingerprint,
            payload_json=snapshot.model_dump_json(),
        )
    )
    world.current_ordinal = configuration.target_week.ordinal
    ...
    session.add(
        AuthoritativeWorldEventModel(
            ...
            command_id=command.command_id,
            week_ordinal=configuration.target_week.ordinal,
            event_kind="season_transition_completed",
            payload_json=event_payload,
        )
    )
    session.flush()

    if fault_at == "after_publication":
        raise RuntimeError("fault after ordinary Season Transition publication")
    ...
    change_summary = {
        "kind": SEASON_TRANSITION_SAVED_REVISION_KIND,
        "summary": (
            f"Closed season {2000 + configuration.completed_week.season_index}/"
            f"{2001 + configuration.completed_week.season_index} and opened "
            f"{2000 + configuration.target_week.season_index}/"
            f"{2001 + configuration.target_week.season_index} Week 1"
        ),
        "completed_week": configuration.completed_week.model_dump(mode="json"),
        "target_week": configuration.target_week.model_dump(mode="json"),
    ...
    if fault_at == "after_revision":
        raise RuntimeError("fault after ordinary Season Transition revision")
    ...
    claimed = session.execute(
        ...
        .values(
            base_revision_id=command.season_saved_revision_id,
    ...
    branch.saved_head_revision_id = command.season_saved_revision_id
    audit_payload = {
        "command_id": command.command_id,
        "request_fingerprint": command.fingerprint,
        "previous_saved_revision_id": command.expected_saved_revision_id,
        "season_saved_revision_id": command.season_saved_revision_id,
        **change_summary,
    }
    session.add(
        BranchRevisionAuditEventModel(
            ...
            branch_id=command.branch_id,
            saved_revision_id=command.season_saved_revision_id,
            event_kind=SEASON_TRANSITION_AUDIT_EVENT_KIND,
