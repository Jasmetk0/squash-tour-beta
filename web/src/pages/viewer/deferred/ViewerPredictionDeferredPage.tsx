...
export function ViewerPredictionDeferredPage({
  ...
  const statusQuery = useQuery({
    queryKey: ['viewer-prediction-deferred-status', kind, activeRunId],
    queryFn: () => getRunStatusSummary(activeRunId ?? ''),
    enabled: Boolean(activeRunId),
    retry: false,
  })
  const eventsQuery = useQuery({
    queryKey: ['viewer-prediction-deferred-events', kind, activeRunId],
  ...
  const rankingSnapshotsQuery = useQuery({
    queryKey: [
      'viewer-prediction-deferred-ranking-snapshots',
      kind,
      activeRunId,
    ],
  ...
  const raceSnapshotsQuery = useQuery({
    queryKey: ['viewer-prediction-deferred-race-snapshots', kind, activeRunId],
    queryFn: () => listRaceSnapshots(activeRunId ?? ''),
    enabled: Boolean(activeRunId),
    retry: false,
  })
  const finalsQuery = useQuery({
    queryKey: ['viewer-prediction-deferred-finals', kind, activeRunId],
    ...
    >
      ...
      >
        ...
        <p className="subtitle">
          Outputs remain deferred. This page only shows safe source availability
          from the active Viewer run.
        </p>
        {isLoadingMetadata ? (
