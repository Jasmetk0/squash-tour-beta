...
def _ranking_row_count(path):
    with sqlite3.connect(path) as connection:
        return connection.execute("SELECT COUNT(*) FROM official_ranking_candidates").fetchone()[0]


def _initial_world_row_count(path):
    with sqlite3.connect(path) as connection:
        return connection.execute("SELECT COUNT(*) FROM initial_world_states").fetchone()[0]
...
def test_production_pool_to_owned_world_derived_ranking_save_reopen_restore(tmp_path):
    ...
    with ApiServer(database_url=f"sqlite:///{db}") as server:
        ...
        assert _post_headers(world_root, adoption, {
            "X-Initial-World-Preview-Fingerprint": preview["fingerprint"]}) == (201, original_owned)
        assert _initial_world_row_count(db) == before_retry == 1
        pool.unlink()
        assert _post_headers(world_root, adoption, {
            "X-Initial-World-Preview-Fingerprint": preview["fingerprint"]}) == (201, original_owned)
        assert _post_headers_result(world_root, adoption | {"best_n": 8}, {
            "X-Initial-World-Preview-Fingerprint": preview["fingerprint"]})[0] == 409
        assert _post_headers_result(world_root, adoption, {
            "X-Initial-World-Preview-Fingerprint": "0" * 64})[0] == 409
        ...
        def ranking_player(player):
            ...
            return {"player_id": player["player_id"], "tie_break_token": hashlib.sha256(identity.encode()).hexdigest(),
                    "tour_entry_week": {"season_index": 0, "week": 1}, "retired": False}

        derived_players = [ranking_player(player) for player in original_owned["players"]]
        forged_base = {"kind": "initial_ranking.v1", "command_id": "forged", "run_id": run_id,
            "branch_id": branch_id, "target_week": {"season_index": 0, "week": 1},
            "policy": original_owned["policies"][0], "players": derived_players, "discipline": "none",
            "initial_world_fingerprint": preview["fingerprint"], "audit": preparation["audit"]}
        for index, mutation in enumerate((
            {"players": []},
            {"policy": {"policy_id": "forged", "best_n": 1,
                        "tie_break_version": "result_profile_age_previous_token.v1"}},
            {"players": [derived_players[0] | {"tie_break_token": "forged"}, *derived_players[1:]]},
        )):
        ...
        runner = RankingWeekCommandRunner(server.app.state.runtime.repository._session_factory)
...
def test_first_adoption_rejects_source_change_after_preview(tmp_path):
    ...
    with ApiServer(database_url=f"sqlite:///{db}") as server:
        server.app.state.initial_player_pool_config_path = pool
        assert _request("POST", server.base_url + "/admin/players/initial-pool/generate", {
            "season": "2000/2001", "seed": 723, "target_pool_size": 12, "dry_run": False})[0] == 200
        ...
        payload = {"command_id": "stale-source", "source_season": "2000/2001", "bootstrap_seed": 44,
                   "audit_label": "Admin", "audit_reason": "Verify stale source", "best_n": 9}
        preview = _request("POST", root + "/preview", payload)[1]
        assert _request("POST", server.base_url + "/admin/players/initial-pool/generate", {
            "season": "2000/2001", "seed": 999, "target_pool_size": 5, "dry_run": False})[0] == 200
        assert _post_headers_result(root, payload, {
            "X-Initial-World-Preview-Fingerprint": preview["fingerprint"]})[0] == 409
        assert _request("GET", root)[0] == 404
        with sqlite3.connect(db) as connection:
            assert connection.execute("SELECT COUNT(*) FROM initial_world_states").fetchone()[0] == 0
