...
Package identity is the globally unique stable `package_id`; `package_type` is an
immutable property of that identity, not part of the identity key. Each immutable source
version has a positive version, schema version, deterministic content fingerprint,
...
Automatic resolution additionally requires the reviewed target entity kind and technical
entity schema version to satisfy the reference contract; an exact ID with an incompatible
kind or schema fails closed rather than being offered as a resolution.
...
Receipts retain the reviewed document/preview, actual selection, resolutions, source-to-
Run-local mapping and exact historical result state, so retries never substitute later live
state.
