...
def _setup(server):
    ...
    payload = {"expected_head_checkpoint_id": checkpoint["checkpoint_id"], "command_id": "api-week", "audit_reason": "test", "explicit_confirmation": True}
    url = f"{server.base_url}/admin/runs/source/branches/{branch_id}/simulate-next-week"
...
def test_admin_branch_next_week_success_replay_and_conflicts(tmp_path):
...
def test_admin_branch_next_week_not_found_cases(tmp_path, case):
    database_url = f"sqlite:///{tmp_path / f'404-{case}.db'}"
    with ApiServer(database_url=database_url) as server:
        branch_id, head, url, payload = _setup(server); repo = _repo(database_url)
        if case == "product": url = f"{server.base_url}/admin/runs/missing/branches/{branch_id}/simulate-next-week"
        elif case == "branch": url = f"{server.base_url}/admin/runs/source/branches/missing/simulate-next-week"
        elif case == "legacy":
...
def test_admin_branch_next_week_conflict_cases(tmp_path, case):
    ...
    with ApiServer(database_url=database_url) as server:
        ...
        with repo._session_factory.begin() as session:
            ...
            if case == "run-inactive": container.status = "inactive"
                ...
                url = f"{server.base_url}/admin/runs/other/branches/{branch_id}/simulate-next-week"
...
def test_admin_branch_next_week_validation_and_no_executable_week(tmp_path):
    ...
    with ApiServer(database_url=database_url) as server:
        ...
        assert _request("POST", url, {**payload, "command_id": "no-week"})[0] == 400


def test_admin_branch_next_week_internal_failure_rolls_back(tmp_path, monkeypatch):
