...
def preflight_season_builder(
    ...
    authoritative_diff_summary = {
        ...
        "template_conflict_summary": build_template_conflict_summary_preview(template_slot_conflict_preview),
    }
    template_conflict_summary = authoritative_diff_summary.get("template_conflict_summary")
    preflight_template_conflict_diagnostics_overview = build_template_conflict_diagnostics_overview(
        preflight_preview=template_slot_conflict_preview,
        preflight_summary=template_conflict_summary if isinstance(template_conflict_summary, dict) else None,
    )
    ...
    return SeasonBuilderPreflightResponse(
        ...
        template_conflict_diagnostics_overview=preflight_template_conflict_diagnostics_overview,
        validation_warnings=warnings,
        validation_errors=errors,
...
def post_season_builder_dry_run_build_contract(
    ...
    if payload.source_type != "season_template":
        ...
        if selected is None:
            ...
            for index, slot in enumerate(selected.slots, start=1):
                ...
                candidate_id, candidate_identity_key = build_candidate_identity(
                    source_template_id=payload.source_template_id,
    ...
    dry_run_template_conflict_summary = dry_run_result_preview.get("template_conflict_summary")
    dry_run_template_conflict_diagnostics_overview = build_template_conflict_diagnostics_overview(
        dry_run_preview=template_slot_conflict_preview,
        dry_run_summary=(
            dry_run_template_conflict_summary
            if isinstance(dry_run_template_conflict_summary, dict)
            else None
        ),
    )
    blocking_count = 0
    ...
    return SeasonBuilderDryRunBuildResponse(
        ...
        template_slot_validation_preview=template_slot_validation_preview,
        template_slot_conflict_preview=template_slot_conflict_preview,
        template_conflict_diagnostics_overview=dry_run_template_conflict_diagnostics_overview,
        validation_errors=errors,
        validation_warnings=warnings,
        audit_preview={
...
def post_season_builder_future_apply_request_validation_preview(
    ...
    disabled_execution_contract_summary = build_disabled_execution_contract_summary(
        future_apply_reference_contract=future_apply_reference_contract,
        future_apply_request_validation_preview=future_apply_request_validation_preview,
    ...
    final_guarded_apply_readiness_checklist = build_final_guarded_apply_readiness_checklist(
    ...
    future_apply_execution_decision_summary = build_future_apply_execution_decision_summary(
    ...
    return SeasonBuilderFutureApplyRequestValidationPreviewResponse(
        ...
        guarded_apply_execution_gate_specification=guarded_apply_execution_gate_specification,
        future_apply_execution_boundary_contract=future_apply_execution_boundary_contract,
        future_apply_execution_decision_summary=future_apply_execution_decision_summary,
        audit_preview={
