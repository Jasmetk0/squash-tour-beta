...
Unranked Editions run the normal match/result/history pipeline, but their persisted point-award package contains zero MSA/race awards and therefore inserts no ranking contribution (the current ranking foundation has not yet implemented Best N storage).
