...
def test_authoritative_simulation_http_guards_retry_and_close(tmp_path):
    ...
    with server:
        ...
        before_preflight = Path(server.app.state.runtime.repository._engine.url.database).read_bytes()
        ...
        assert "season_transition_atomic_writer_not_implemented" in season_preflight["implementation_gaps"]
        assert len(season_preflight["preflight_fingerprint"]) == 64
        assert Path(server.app.state.runtime.repository._engine.url.database).read_bytes() == before_preflight
        base = {
            "command_id": "sf",
            "run_id": run_id,
            "branch_id": branch_id,
