...
@pytest.mark.smoke
def test_pre_draw_withdrawal_shortcut_fails_closed_without_mutation(tmp_path: Path) -> None:
    service = make_draw_service(tmp_path)
    event_id = persist_entry_list(service, seed=123)
    entry_list = service.entry_list_service.get_entry_list(event_id=event_id).entry_list
    assert entry_list is not None
    ...
    assert service.get_pre_draw_withdrawal_replacements(event_id=event_id) == ()
    assert service.get_draw_package(event_id=event_id).draw_package_exists is False

    preview = service.generate_draw_package(
        event_id=event_id,
        request=DrawGenerateRequest(seed=1201, dry_run=True),
    ).draw_package
    assert preview is not None
    assert preview.metadata.pre_draw_withdrawals_fingerprint is None
    assert withdrawn.player_id in {
        slot.player_id
        for slot in preview.main_draw.slots
        if slot.player_id is not None
    }


def test_pre_draw_withdrawal_stays_locked_after_draw(tmp_path: Path) -> None:
    service = make_draw_service(tmp_path)
    event_id = persist_entry_list(service, seed=123)
    entry_list = service.entry_list_service.get_entry_list(event_id=event_id).entry_list
    assert entry_list is not None
