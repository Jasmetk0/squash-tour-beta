...
class TournamentDrawInputAuthority(FrozenInput):
    ...
    released_wild_card_slot_count: int = Field(
        default=0,
        ge=0,
        exclude_if=lambda value: value == 0,
    )
    ...
    def validate_structure(self) -> "TournamentDrawInputAuthority":
        ...
        if self.schema_version in {
            ...
            or self.released_wild_card_slot_count
        ):
            raise ValueError(
        ...
        if self.schema_version in {
            ...
            if self.schema_version == "tournament_draw_input_authority.v9":
                if (
                    len(self.wild_card_player_ids)
                    + self.released_wild_card_slot_count
...
class TournamentDrawInputAuthorityBuilder:
    ...
    def build_frozen_ordinary_fallback(
        *,
        ...
        released_wild_card_slot_count = (
            previous.released_wild_card_slot_count
            + (1 if released_wild_card_slot else 0)
        )
        return TournamentDrawInputAuthority(
            schema_version=(
                "tournament_draw_input_authority.v9"
                if released_wild_card_slot_count
            ...
            released_wild_card_slot_count=released_wild_card_slot_count,
    ...
    def build_lucky_loser_vacancy(
        *,
        ...
        if previous.direct_main_player_ids.count(withdrawn_player_id) != 1:
            raise ValueError(
                "First Lucky Loser vacancy slice requires one direct Main withdrawal"
            )
        ...
        return TournamentDrawInputAuthority(
            ...
            released_wild_card_slot_count=previous.released_wild_card_slot_count,
            withdrawn_player_ids=tuple(
                sorted(set((*previous.withdrawn_player_ids, withdrawn_player_id)))
            ),
            main_seed_player_ids=tuple(main_seed_players),
