...
The tournament bridge now freezes a v4 Run/Branch/week authority bundle and consumes a canonical executable DAG built from persisted match IDs, direct slots and `winner_to_match_id` evidence. Explicit schedules can therefore cover complete binary Main Draws beyond four players (the generalized graph is regression-proven with an eight-player/seven-match shape), including multiple events; schedule coverage, identities, feeders, acyclicity, terminals and strictly earlier feeder slots fail closed before sporting mutation. Completion is topology-driven and produces one owned ranking source. Historical v1-v3 four-player readers remain compatible. Qualification currently fails closed because the persisted Match Package explicitly lacks qualifier-winner-to-indexed-placeholder promotion evidence; persisted BYEs likewise fail closed rather than creating a played match until their downstream side mapping is explicit. WC, LL and alternates remain unsupported. Legacy `start_day`, list order and round-name text are never chronology authority.
...
| Tournament flow | generalized persisted topology in `authoritative_run_simulation_driver.py`, result/award services, `owned_tournament_sources.py`; four-player SQLite acceptance plus eight-player graph regression | Complete explicit binary Main Draw DAGs can be frozen and scheduled; production Qualification promotion, BYE advancement, WC/LL/alternates and broader abnormal sources remain fail-closed boundaries |
