...
def confirm(client, kind, command, fingerprint):
    req = request.Request(
        client.base_url + PREFIX + "/prepare/" + kind,
        data=json.dumps(command).encode(),
        method="POST",
        headers={
            "Content-Type": "application/json",
            "X-Ranking-Preview-Fingerprint": fingerprint,
        },
    )
    try:
...
def test_preview_is_read_only_and_confirm_matches_then_retries(api):
    ...
    before = dump(path)
    assert confirm(client, "initial", initial(), expected["fingerprint"]) == (
        201,
        expected,
    )
    assert (
        client.post(PREFIX + "/prepare/initial/preview", json=initial()).json()
        == preview.json()
    )
    assert dump(path) == before
...
def test_week_preview_rolls_back_zero_batch_and_stale_confirm_is_atomic(api):
    ...
    external = RankingZeroVersion.model_validate_json(
        json.dumps(zero | {"zero": zero["zero"] | {"zero_id": "external"}})
    )
    ...
    before = dump(path)
    assert (
        confirm(client, "week", req, preview.json()["candidate"]["fingerprint"])[0]
        == 409
    )
    assert dump(path) == before
    ...
    assert (
        refreshed.json()["candidate"]["fingerprint"]
        != preview.json()["candidate"]["fingerprint"]
    )
    assert dump(path) == before
    assert (
        confirm(client, "week", req, refreshed.json()["candidate"]["fingerprint"])[0]
        == 201
    )
...
def test_preview_failure_and_invalid_confirmation_leave_storage_unchanged(api, case):
    ...
    if case == "wrong_fingerprint":
        ...
        assert client.post(PREFIX + "/prepare/week/preview", json=req).status_code in (
            409,
            422,
        )
    assert dump(path) == before
...
def test_real_run_preview_confirm_save_and_reopen(tmp_path):
    ...
    with ApiServer(database_url=f"sqlite:///{path}") as server:
        run_id, branch_id, initial_revision = _create_run(
            server, display_name="Ranking workflow"
        )
        ...
        with request.urlopen(req) as response:
            assert (
                response.status == 201
                and json.loads(response.read()) == preview["candidate"]
            )
        ...
        assert saved["saved_revision"]["payload"]["content"]["ranking_preparation"][
            "state"
        ]["entries"][0]["receipts"][0]["request_payload_json"]
    with ApiServer(database_url=f"sqlite:///{path}") as reopened:
        ...
        assert (
            _request("GET", root + "/save/preview")[1]["has_unsaved_changes"] is False
        )
        assert (
            _request("GET", root + "/0/1/inputs")[1]["command_audits"][0]["audit"]
            == command["audit"]
        )


@pytest.mark.smoke
def test_authoritative_week_inputs_are_server_resolved_and_preview_is_read_only(
    tmp_path,
):
    from test_saved_revision_history_api import ApiServer, _create_run, _request
    ...
    with ApiServer(database_url=f"sqlite:///{path}") as server:
        run_id, branch_id, revision_id = _create_run(
            server, display_name="Authority workflow"
        )
        root = f"{server.base_url}/admin/runs/{run_id}/branches/{branch_id}/ranking-candidates"
        ...
        authority = {
            ...
            "adopted_by_command_id": "adopt-week-2",
        ...
        status, review = _request("GET", root + "/save/preview")
        assert status == 200 and review["can_save"]
        status, authority_saved = _request(
            "POST",
            root + "/save",
            {
                "expected_draft_version": review["draft_version"],
        ...
        status, preview = _request(
            "POST", root + "/prepare/week/authoritative/preview", request_body
        )
        assert status == 200 and dump(path) == before
        req = request.Request(
            root + "/prepare/week/authoritative",
        ...
        assert _request("POST", root + "/prepare/week/authoritative/preview", forged)[
            0
        ] in (409, 422)
        status, review = _request("GET", root + "/save/preview")
        assert status == 200 and review["can_save"]
        ...
        assert status == 201
        state = saved["saved_revision"]["payload"]["content"]["ranking_preparation"][
            "state"
        ]
    ...
    with ApiServer(database_url=f"sqlite:///{path}") as reopened:
        ...
        assert (
            _request("POST", root + "/prepare/week/authoritative", request_body)[0]
            == 201
        )
        assert dump(path) == before_retry
        restore_url = (
            f"{reopened.base_url}/run-containers/{run_id}/branches/{branch_id}"
            f"/saved-revisions/{revision_id}/restore"
        )
        bad = {
            "expected_head_saved_revision_id": authority_saved["saved_revision"][
                "revision_id"
            ],
...
def test_real_tournament_adoption_candidate_save_reopen_and_restore(tmp_path):
    ...
    with ApiServer(database_url=f"sqlite:///{path}") as server:
        app = server.app
        app.state.season_active_players_config_path = (
            points.active_players_service.active_players_path
        )
        app.state.season_calendar_registry_path = (
            points.calendar_service.calendar_registry_path
        )
        app.state.tournament_templates_config_path = points.template_service.config_path
        app.state.season_matches_registry_path = (
            points.result_service.match_service.matches_path
        )
        app.state.season_event_results_registry_path = (
            points.result_service.results_path
        )
        app.state.season_point_awards_registry_path = points.awards_path
        app.state.points_config_path = points.points_config_path
        run_id, branch_id, initial_revision = _create_run(
            server, display_name="Owned tournament workflow"
        )
        root = f"{server.base_url}/admin/runs/{run_id}/branches/{branch_id}/ranking-candidates"
        players = [
            dict(
        ...
        bootstrap = dict(
            ...
            command_id="owned-bootstrap",
            run_id=run_id,
            branch_id=branch_id,
            target_week={"season_index": 0, "week": 1},
            policy={"policy_id": "owned-policy"},
            players=players,
            discipline="none",
            audit={
                "actor_label": "Admin operator",
                "reason": "Establish pre-tournament baseline",
            },
        )
        assert _request("POST", root + "/prepare/initial", bootstrap)[0] == 201
        if result.season_week > 1:
            ...
            bridge = dict(
                command_id="prepare-pre-event-week",
                tournaments=[],
                corrections=[],
                context=dict(
                    run_id=run_id,
                ...
                audit={
                    "actor_label": "Admin operator",
                    "reason": "Prepare the event completion week",
                },
            )
        ...
        binding = dict(
            ...
            first_publication_week={"season_index": 0, "week": result.season_week + 1},
            validity_weeks=61,
        ...
        weekly_command = dict(
            ...
            audit={
                "actor_label": "Admin operator",
                "reason": "Review and prepare Official candidate",
            },
        )
        before = dump(path)
        status, preview = _request(
            "POST", root + "/prepare/week/preview", weekly_command
        )
        assert status == 200, preview
        assert dump(path) == before
        req = request.Request(
            root + "/prepare/week",
            data=json.dumps(weekly_command).encode(),
            method="POST",
            headers={
                "Content-Type": "application/json",
                "X-Ranking-Preview-Fingerprint": preview["candidate"]["fingerprint"],
        ...
        assert {
            row["player_id"]: row["points"] for row in candidate["snapshot"]["rows"]
        ...
        assert (
            state["tournament_sources"][0]["awards"]["metadata"]["build_fingerprint"]
            == awards.metadata.build_fingerprint
        )
    with ApiServer(database_url=f"sqlite:///{path}") as reopened:
        root = f"{reopened.base_url}/admin/runs/{run_id}/branches/{branch_id}/ranking-candidates"
        ...
        restore_request = {
            "expected_head_saved_revision_id": saved_revision,
            "expected_draft_version": saved["working_draft"]["draft_version"],
            "expected_current_viewer_branch_id": branch_id,
            "explicit_confirmation": True,
        }
        status, restored = _request(
            "POST", restore_root + f"/{initial_revision}/restore", restore_request
        )
        assert status == 201
        ...
        status, _ = _request(
            "POST", restore_root + f"/{saved_revision}/restore", restore_request
        )
        assert status == 201
        assert _request("GET", root + f"/0/{result.season_week + 1}")[1] == candidate
...
def test_confirmation_binds_audit_and_exact_request_as_well_as_snapshot(api):
    ...
    changed = initial() | {
        "audit": initial()["audit"] | {"reason": "Changed after review"}
    }
    before = dump(path)
    req = request.Request(
        client.base_url + PREFIX + "/prepare/initial",
...
def test_result_correction_preview_confirm_history_and_recovery(api):
    ...
    client, factory, path, *_ = api
    assert client.post(PREFIX + "/prepare/initial", json=initial()).status_code == 201
    original = RankingResultVersion.model_validate_json(
        json.dumps(
            dict(
                run_id="run",
                branch_id="empty",
                effective_week={"season_index": 0, "week": 2},
                ...
                result=dict(
                    ...
                    completed_week={"season_index": 0, "week": 1},
                    first_publication_week={"season_index": 0, "week": 2},
    ...
    req = second | {
        "command_id": "correct-award",
        "context": second["context"]
        | {
            "completed_week": {"season_index": 0, "week": 2},
            "target_week": {"season_index": 0, "week": 3},
        },
        "corrections": [
            source["version"]
            | {
                "effective_week": {"season_index": 0, "week": 3},
                "previous_fingerprint": source["fingerprint"],
                "result": source["version"]["result"]
                | {
                    "qualification_points": 20,
                    "main_points": 200,
                    "source_fingerprint": "appeal",
                },
            }
        ],
    }
    ...
    status, candidate = confirm(
        client, "week", req, preview.json()["candidate"]["fingerprint"]
    )
    assert status == 201
    ...
    for index, target_state in enumerate((prior_state, state)):
        ...
        with factory.begin() as session:
            ...
            restore_ranking_revision_state(
                session,
                target_state.model_dump_json(),
                expected_fingerprint=target_state.fingerprint,
                expected_current_fingerprint=current.fingerprint,
                command_id=f"corrected-recovery-{index}",
                run_id="run",
                branch_id="empty",
            )
        assert capture(factory).fingerprint == target_state.fingerprint
    assert (
        client.get(PREFIX + "/0/3/sources").json()["sources"][0]["version"] == corrected
    )
    before = dump(path)
...
def test_invalid_result_correction_rolls_back_zero_batch(api, damage):
    ...
    client, factory, path, *_ = api
    assert client.post(PREFIX + "/prepare/initial", json=initial()).status_code == 201
    source = RankingResultVersion.model_validate_json(
        json.dumps(
            dict(
                run_id="run",
                branch_id="empty",
    ...
    assert (
        client.post(
            PREFIX + "/prepare/week", json=weekly() | {"zero_versions": []}
        ).status_code
        == 201
    )
    req = weekly()
    ...
    req["context"].update(
        completed_week={"season_index": 0, "week": 2}, target_week=target
    )
    req["zero_versions"][0]["effective_week"] = target
    req["zero_versions"][0]["zero"]["effective_week"] = target
    correction = source.model_dump(mode="json") | {
        "effective_week": target,
        "previous_fingerprint": source.fingerprint,
    }
    if damage == "timing":
