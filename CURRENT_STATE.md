...
That real Q history then feeds a chronological Lucky Loser vacancy + fill pair on the source Branch. Materialized fork remap must rebind the Q terminal and candidate-elimination result fingerprints into the target LL order, install the target-owned revision chain exactly, allow a second LL vacancy/fill on the target Branch, restore back to the fork root, and preserve exact retry identity for the original LL fill command. The final target component fingerprint must return to the materialized fork-root fingerprint after restore.
