...
The current narrow driver automatically adopts every persisted supported four-player
Main Draw in the current Run/Branch/week, derives one deterministic global timeline
from authoritative calendar `start_day` and stored draw-round topology, preserves
each match identity, and closes each tournament exactly once through the existing
completion, award and `OwnedTournamentRankingSource` contracts. Missing or ambiguous
scheduling evidence fails closed. Explicit authoritative Admin routes expose
...
General draws, Qualification, WC/LL, Entries, AI, scheduling beyond this narrow
calendar/day and round projection, and health remain Gate 3 work.
